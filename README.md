...
Nếu bạn có file dump SQL, import vào database `apple_store` bằng phpMyAdmin hoặc:

```bash
mysql -u root apple_store < duong-dan-den-file.sql
```
...
- Ảnh sản phẩm demo nằm tại `storage/app/public/products/demo/`; chạy `php artisan storage:link` để phục vụ qua `/storage/...`. Nếu thiếu ảnh, site dùng placeholder tại `public/images/placeholders/product-placeholder.svg`.
