...
Sinh viên nộp file **`xampp-lite-web-build.zip`**. Giải nén toàn bộ nội dung vào thư mục `www/` của XAMPP-Lite (file `artisan` nằm ngay trong `www/`).
...
### Bước 2 — Giải nén `xampp-lite-web-build.zip` vào `www/`

Giải nén **`xampp-lite-web-build.zip`** — copy **toàn bộ** nội dung (hoặc thư mục sau giải nén) vào `www/` — file `artisan` nằm **ngay trong** `www/`, không tạo subfolder.
...
Nén thủ công thư mục `xampp-lite-web-build` thành **`xampp-lite-web-build.zip`** (chuột phải → **Nén vào...** / 7-Zip), rồi nộp file zip.
